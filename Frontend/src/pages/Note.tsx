...
import { noteService, Note, CreateNoteRequest } from "@/services/note";

const NotePage = () => {
  const [entities, setEntities] = useState<Note[]>([]);
  ...
  const [editingEntity, setEditingEntity] = useState<Note | null>(null);
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const response = await noteService.search(searchTerm, currentPage, 10);
      setEntities(response.content);
      setTotalPages(response.totalPages);
      setError(null);
    } catch (err) {
  ...
  const handleCreate = async () => {
    try {
      ...
      setFormData({ noteTitle: "", description: "" });
      loadEntities();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await noteService.update(editingEntity.id, formData);
  ...
  const openEditDialog = (entity: Note) => {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            <div>
              ...
              <p className="text-slate-400">Manage your notes</p>
            </div>
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
              ...
              <DialogContent className="bg-slate-800 border-slate-700">
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="noteTitle" className="text-slate-300">Note Title</Label>
                    <Input
                      id="noteTitle"
              ...
              <Card key={entity.id} className="bg-slate-800/50 border-slate-700 hover:bg-slate-800/70 transition-colors">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <CardTitle className="text-white text-lg">{entity.noteTitle}</CardTitle>
                        ...
                        <AlertDialogContent className="bg-slate-800 border-slate-700">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className="text-slate-400">
                              Are you sure you want to delete "{entity.noteTitle}"? This action cannot be undone.
